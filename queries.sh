...
echo "$($PSQL "SELECT DISTINCT name FROM teams full JOIN games AS game_a ON game_a.opponent_id = teams.team_id LEFT JOIN games AS game_b ON game_b.winner_id = teams.team_id WHERE (game_b.year = 2014 AND game_b.round='Eighth-Final') OR (game_a.year = 2014 AND game_a.round='Eighth-Final') ORDER BY name ")"
